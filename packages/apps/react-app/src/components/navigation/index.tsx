    ...
    return <div>
        {
            navs?.map((item) => <NavLink
                ...
                className={({ isActive }) => (
                    ...
                    (isActive || (getParentPath() === item.path) ? "font-extrabold text-black border-b-4 border-blue-600" : "text-slate-500")
                )}
            >
                {item.title}
            </NavLink>)
...
const MenuAlarm = () => <div className='flex mr-10 gap-6'>
    ...
    <div className='flex justify-center items-center'>
        <img className='w-7 h-7' src="https://picx.zhimg.com/v2-abed1a8c04700ba7d72b45195223e0ff_l.jpg?source=32738c0c" />
    </div>

</div>

function Navigation({ hide }: Props) {
    return (
        <div className='bg-white w-screen shadow top-0 sticky overflow-hidden h-13 box-border'>
            <div className=' max-w-7xl min-w-6xl mx-auto my-0 flex justify-center'>
                <div className={`${hide ? "top-0" : "-top-13"}  w-full flex flex-col items-center justify-between relative transition-all duration-300`}>
